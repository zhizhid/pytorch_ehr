...
def model_tune(dlm_code, embdim_exp, hid_exp, layers_n, dropout, ct_code , opt_code , l2_exp , lr_exp , eps_exp, k ,bt_size):
    ...
    if int(dlm_code)<3:
      if int(ct_code) ==1:
        ...
        if int(ct_code)==3:
           cnn_grp = embed_dim
           cell_type= 'depthwiseCNN'
           ch_out = hidden_size = embed_dim * k_size
           
        else:
           cnn_grp = int(ct_code)
           cell_type= str(cnn_grp)+'group'        
        ch_out = hidden_size
        dil_depth = n_layers
